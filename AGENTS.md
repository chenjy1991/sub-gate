...
pnpm db:seed          # 执行 migration + 初始化种子数据
pnpm db:reset         # 删除本地数据库后重新初始化
```
...
当前已配置 `node:test + tsx` 的最小测试框架，测试文件位于 `tests/`。
...
│   │   ├── schemas.ts                # Route Handler 公共 schema
...
├── package.json
├── drizzle.config.ts
├── drizzle/                          # Drizzle migration SQL 与元数据
├── Dockerfile
├── docker-compose.yml
└── specs/                            # 功能规格文档
```
...
- 鉴权由 `middleware.ts` 统一处理登录态，API 内优先通过 `lib/api/auth.ts` 获取当前用户并校验权限
- 入参校验优先通过 `lib/api/validation.ts` 与 Zod schema 统一处理
...
- **JWT 配置**：开发环境可回退默认密钥；生产环境必须显式配置 `JWT_SECRET`
...
| 012 | 用户 Email 字段 + 个人设置 | `specs/012-user-email/` |

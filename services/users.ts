...
export function createUser(data: { username: string; email: string; password: string; nickname?: string; status?: number; roleIds?: string[] }): Promise<void> {
  return request('/api/user/create', { method: 'POST', body: JSON.stringify(data) })
}

export function updateUser(data: { id: string; username?: string; email?: string; password?: string; nickname?: string; status?: number; roleIds?: string[] }): Promise<void> {
